...
void exporter_orde_lexinv(AVLUsine *n, const char *opt, FILE *f)
{
    ...
    if (strcmp(opt, "max") == 0) { // trouve mode pour retourner bonne valeur / 1000 car v 1.1 en M.m3
        valeur = n->data.capacite_max / 1000.0;
...
int exporter_histogramme(AVLUsine *racine_usines, char *opt, char *chemin_sortie_dat)
{
    ...
    fprintf(f, "id_usine;%s\n", opt); // en-tête du fichier
