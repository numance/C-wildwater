...
// =========================================================
// AVL ACTEURS : fonctions de base équilibre et rotation
// =========================================================
...
// =========================================================
// Réseau d’acteurs : création, liens, libération
// =========================================================
...
void maj_max_troncon(MaxFuiteTroncon* maxT, char *id_amont, char *id_aval, double fuite) { 
    ...
    // maj si troncons comparé sup
    if (fuite > maxT->fuite_max) {
        maxT->fuite_max = fuite;

        strncpy(maxT->id_amont, id_amont, TAILLE_ID - 1);
        maxT->id_amont[TAILLE_ID - 1] = '\0';

        strncpy(maxT->id_aval, id_aval, TAILLE_ID - 1);
        maxT->id_aval[TAILLE_ID - 1] = '\0';
    }
}

void propager_fuites(Acteur *acteur, double vol_entree, double *fuites_totales, MaxFuiteTroncon* maxT) {
    ...
    for (LienActeur *l = acteur->premier_enfant; l; l = l->suiv) { 
        ...
        *fuites_totales += fuite; 
        ...
        // BONUS : mise à jour du tronçon (amont=acteur->id, aval=l->enfant->id)
        maj_max_troncon(maxT, acteur->id, l->enfant->id, fuite);

        propager_fuites(l->enfant, aval, fuites_totales, maxT); // appel recursif enfant a aval
    }
}


double calculer_fuites_usine(char *id_usine, AVLUsine *racine_usines, AVLActeur *racine_index_acteurs, MaxFuiteTroncon* maxT)
{
    ...
    if (!na) {
        fprintf(stderr, "Erreur : acteur \"%s\" introuvable (AVL acteurs).\n", id_usine);
        return -1.0;
    }


    double fuites = 0.0; // met fuite a 0
    propager_fuites(na->acteur, vol_traite, &fuites, maxT); // calcul pour tout les enfant de l'usine + track max tronçon
    return fuites;
}
...
int exporter_fuites_usine(AVLUsine *racine_usines, char *id_usine, char *chemin_sortie_dat, double leaks, MaxFuiteTroncon* maxT) {
    ...
    if (taille == 0) {
        // bonus : on sort aussi amont/aval et la fuite max (meme unité)
        fprintf(f, "id_usine;leaks;amont_fuite_max;aval_fuite_max;leak_max\n");
    }
    ...
    // Écriture de la ligne de résultat (unité : comme précédemment /1000)
    leaks = leaks / 1000.0;

    fprintf(f, "%s;%.3f;%s;%s;%.3f\n", u->data.id_usine, leaks, maxT->id_amont, maxT->id_aval, maxT->fuite_max);
