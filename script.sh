...
if [ "$mode" = "histo" ]; then
    ...
    if [ $RET -ne 0 ]; then
        ...
        tail -n +2 "$OUT_DATA" | sort -t';' -k2,2n | head -n 50 >> "$TMP_SMALL" # ignore 1ere ligne et tri numérique 2eme colonne et copie les 50 premier (ordre croissant)

        echo "Préparation des données (10 plus grandes valeurs) : $TMP_LARGE"


        head -n 1 "$OUT_DATA" > "$TMP_LARGE" # créé le fichier ou écrasé et copie 1er ligne de histo (nom colonne)


        tail -n +2 "$OUT_DATA" | sort -t';' -k2,2n | tail -n 10 >> "$TMP_LARGE" # ignore 1ere ligne et tri numérique 2eme colonne et copie les 10 dernier
