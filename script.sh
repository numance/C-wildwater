...
case "$mode" in # différent cas si mode = histo ou leaks sinon erreur
    histo)
        ...
        OUT_PNG_PETIT="${OUT_BASE}_50_petit.png"  # image 50 plus petites
        OUT_PNG_GRAND="${OUT_BASE}_10_grand.png"  # image 10 plus grandes

        TMP_PETIT="tmp_${OUT_BASE}_50_petit.dat" # fichier temporaire 50 plus petites
        TMP_GRAND="tmp_${OUT_BASE}_10_grand.dat" # fichier temporaire 10 plus grandes
        ;;
...
if [ "$mode" = "histo" ]; then
    ...
    if [ $RET -ne 0 ]; then
        ...
        echo "Préparation des données (10 plus grandes valeurs) : $TMP_GRAND"
...
EOF
        ...
        echo "Génération de l'image PNG (10 plus grandes) : $OUT_PNG_GRAND"
...
EOF
        ...
        rm -f "$TMP_PETIT" "$TMP_GRAND" # supprime les fichiers temporaires (plus propre)
    fi
